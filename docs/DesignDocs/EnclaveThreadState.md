...
oe_sgx_register_td_host_signal(           RUNNING
  td2, SIGUSR1);                           oe_sgx_td_unmask_interrupt(td2);
  |                                         |
  v                                         |
OCALL with SIGUSR1                          |
  |                                         |
...
- Checking the interrupt request
  ...
  - `interrupt_unmasked`
    ...
    internal API `oe_sgx_td_unmask_interrupt`. If the check passes, the runtime
...
- Checking after interrupt handling
  ...
  If the check passes, the runtime clears the `interrupted` flag. Also, the runtime always
